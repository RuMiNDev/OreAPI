...
getgenv().IS_Ore_LOADED = false
local oldr = request 
getgenv().request = function(options)
	if options.Headers then
		options.Headers["User-Agent"] = "Ore/RobloxApp/2.1"
	else
		options.Headers = {["User-Agent"] = "Ore/RobloxApp/2.1"}
	end
...
getgenv().require = function(scr) -- not mine
	...
	if (type(scr) == "number") then 
		if not game:GetObjects('rbxassetid://' .. scr)[1] then 
			warn("[ Ore ]: Require failed: invalid asset ID")
			return 
		end
		if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
			if game:GetObjects('rbxassetid://' .. scr)[1].Name == "MainModule" then 
				if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
					...
					warn("[ Ore ]: Require failed: cant require a modulescript with no code")
				end
			else 
				warn("[ Ore ]: Require failed: require asset id failed")
			end
		end
		return
	end
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "Ore", "getaffiliateid function test failed")
end)
...
function Bridge:InternalRequest(body, timeout)
	...
	if body.Url then
		...
		error("[Ore Error]: Unknown error", 2)
		return
	end
	...
	if success and result then
		error("[Ore Error]: " .. tostring(result), 2)
	end

	error("[Ore Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "[Ore Error]: webServer connection failed:  " .. self.serverUrl,
		StatusCode = 599;
...
if not shared.vulnsm then 
	...
	getgenv().rconsoleclear = function()
		Bridge:rconsole("cls") 
		rconsolesettitle("Ore is NOT fat!")
	end
		...
		if type(real) == "userdata" then
			...
			meta.__index = function(s,k)
				if table.find(vulnFuncTbl, k) then 
					return function()
						error("[ Ore ]: "..tostring(k).." isn't available.")
					end
...
print("[ Ore ]: Functions Added")
getgenv().IS_Ore_LOADED = true
